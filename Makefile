...
DEFAULT_NDK_ROOTS := \
  ...
  $(wildcard $(HOME)/Library/Android/sdk/ndk/*) \
  $(HOME)/android-ndk-cache/android-ndk-r29 \
  $(wildcard /opt/homebrew/Caskroom/android-ndk/*/AndroidNDK*.app/Contents/NDK)
NDK_ROOT ?= $(or $(ANDROID_NDK_HOME),$(ANDROID_NDK_ROOT),$(lastword $(sort $(wildcard $(DEFAULT_NDK_ROOTS)))))
...
HOST_BIN := $(BUILD)/smt878u-ionstack-reroot
WINDOWS_HOST_BIN := $(BUILD)/smt878u-ionstack-reroot.exe
